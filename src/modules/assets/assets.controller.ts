...
import { valueContainsAssetId } from './asset-reference-validation';
...
export class AssetsController {
  ...
  async remove(@Req() req: Request, @Param() params: UuidIdParamDto) {
    ...
    const [contents, explanations, choiceAnswers, solutionAnswers, layoutElements, exportJobs] = await this.prisma.withTenant(tenantId, (tx) =>
      Promise.all([
  ...
  async cleanup(
    ...
    const [assets, contents, explanations, choiceAnswers, solutionAnswers, layoutElements, exportJobs] = await this.prisma.withTenant(tenantId, (tx) =>
      Promise.all([
