...
describe('ExportJobsController', () => {
  ...
  it('returns service unavailable without mutating the job when queue access fails during cancel', async () => {
    ...
    const controller = new ExportJobsController(prisma, {}, makeAudit());
    jest.spyOn<any, any>(controller as any, 'openQueue').mockRejectedValue(new Error('redis down'));
  ...
  it('returns service unavailable before cleanup mutates stale jobs when queue access fails', async () => {
    ...
    prisma.withTenant
      ...
      .mockImplementationOnce(async (_tenantId: string, fn: any) =>
        fn({
          exportJob: { findMany: jest.fn().mockResolvedValue([{ id: 'job-1', status: 'pending' }]) }
        })
      )
      .mockImplementationOnce(async (_tenantId: string, fn: any) =>
        fn({
