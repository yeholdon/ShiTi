...
import { EXPORT_JOBS_QUEUE, QUEUE_CONNECTION } from '../../../src/queue/queue.constants';
...
import { hasTestFault } from '../../../src/domain/export-jobs/test-faults';
...
export class ExportJobsController {
  ...
  async getResult(@Req() req: Request, @Res() res: Response, @Param() params: UuidIdParamDto) {
    ...
    if (asset.storageKey.startsWith('/')) {
      try {
        const body = await fs.readFile(asset.storageKey);
