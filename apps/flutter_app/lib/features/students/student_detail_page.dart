...
class StudentDetailPage extends StatelessWidget {
  ...
  Future<void> _editStudent(
    ...
    final updated = await showEditStudentDialog(context, student: student);
    ...
    Navigator.of(context).pushReplacementNamed(
      ...
      arguments: StudentDetailArgs(
        ...
        flashMessage: '已更新 ${updated.name} 的学生档案。',
  ...
  Future<void> _setStudentArchived(
    BuildContext context,
    StudentWorkspaceRecord student, {
    required bool archived,
  }) async {
    try {
      final updated = await AppServices.instance.studentRepository
          .setStudentArchived(student.id, archived: archived);
      if (!context.mounted) {
        return;
      }
      Navigator.of(context).pushReplacementNamed(
        ...
        arguments: StudentDetailArgs(
          ...
          flashMessage: archived
              ? '已归档 ${updated.name}，学生列表默认不再显示。'
              : '已恢复 ${updated.name}，学生列表会重新显示。',
          sourceModule: sourceModule,
          sourceRecordId: sourceRecordId,
          sourceLabel: sourceLabel,
        ),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            '${archived ? '归档' : '恢复'}学生失败：${error.message}（HTTP ${error.statusCode}）',
          ),
        ),
      );
    } catch (error) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('${archived ? '归档' : '恢复'}学生失败：$error')),
      );
    }
  }
            ...
            trailing: Wrap(
              ...
              children: [
                ...
                FilledButton.tonalIcon(
                  ...
                  label: Text(student.isArchived ? '恢复档案' : '归档档案'),
                ),
                FilledButton.tonalIcon(
                  style: FilledButton.styleFrom(
                    foregroundColor: Colors.white,
                      ...
                      children: [
                        ...
                        if (student.isArchived) ...[
                          ...
                          const WorkspaceMessageBanner.warning(
                            title: '档案已归档',
                            message: '学生列表默认隐藏已归档档案。你仍可以在当前详情页恢复或继续查看历史记录。',
                          ),
                        ],
                      ],
                    );
