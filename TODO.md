...
- [x] Flutter: 第二批把文档页也接入新版桌面工作台壳层，让题库、文档、导出三条主工作流在独立总览模式下统一到同一套左侧模块导航与顶部状态条。
- [x] Flutter: 第二批把课堂页从概览页推进成“课堂列表 + 当前课堂摘要”工作页，和学生、班级页对齐成同一套真实桌面工作台结构。
- [x] Flutter: 学生、班级工作页补进“历史成绩 / 错题跟进 / 学习习惯 / 班级规模 / 当前课堂 / 资料联动”等字段层，让首版结构更贴近新版数据模型。
- [x] Flutter: 课堂工作页补进“关联班级 / 主资料 / 课后反馈 / 课后任务”字段层，让课堂页也开始贴近新版课堂数据模型。
- [x] Flutter: 学生、班级、课堂三页新增统一“联动工作流”面板，把学生画像、班级组织、课堂反馈和文档资料串成同一条教学链路。
- [x] Flutter: 学生、班级、课堂三页右侧摘要 rail 补进可点击的跨模块快捷动作，让三页不仅有联动说明，也能直接互相跳转。
- [x] Flutter: 学生、班级、课堂三页把快捷动作改成带当前对象语义的入口文案，如“查看当前班级 / 打开当前资料 / 回写学生反馈”，让跨模块跳转更像真实工作流。
- [x] Flutter: 学生、班级、课堂三页开始支持带上下文的跨模块跳转，切页时会把当前学生/班级/课堂语义带到目标模块，而不是只做模块级切换。
- [x] Flutter: 班级页、课堂页跳到学生页时开始携带代表学生定位，快捷动作也改成当前学生语义，避免只落到学生模块却没有具体对象。
- [x] Flutter: 学生、班级、课堂跳到文档页时开始携带“当前资料用途”高亮，让文档页知道这是学生跟进资料、班级资料还是课堂资料，而不只是聚焦同一份文档。
- [x] Flutter: 学生页开始区分“班级回看 / 课堂回看”两类上下文高亮，让从班级、课堂跳进学生画像时能直接看见当前来源和当前对象。
- [x] Flutter: 班级页、课堂页也开始支持来源高亮，让学生回看、班级回看、课堂回看三条链路在三页之间形成对称的上下文表达。
- [x] Flutter: 学生模块新增首版详情页，学生工作页现在可以从名册卡片和右侧摘要 rail 进入对象级档案，而不只停在列表工作页。
- [x] Flutter: 班级模块新增首版详情页，班级工作页现在可以从班级卡片和右侧摘要 rail 进入对象级档案，继续查看班级结构、课堂安排和资料联动。
- [x] Flutter: 课堂模块新增首版详情页，课堂工作页现在可以从课堂卡片和右侧摘要 rail 进入对象级档案，继续查看班级安排、资料联动、反馈与课后任务。
- [x] Flutter: 学生、班级、课堂详情页之间的跨模块入口已经升级为详情对详情跳转，不再只回到列表页承接上下文。
- [x] Flutter: 学生、班级、课堂详情页右侧摘要 rail 已补齐“关联对象”区，首屏可直接理解上下游对象链路。
- [x] Flutter: 学生、班级、课堂详情页已补齐“返回来源”语义，从详情页继续跳转后可优先回到来源对象，而不是统一回列表页。
- [x] Flutter: 学生详情页新增首版“成绩记录”分区，开始承接学生模型里的历史成绩轨迹与单次测评洞察。
- [x] Flutter: 班级详情页新增首版“成员分层”分区，开始承接班级模型里的成员规模与分层跟进结构。
...
- [x] Flutter: 班级详情页的“课堂时间线”和课堂详情页的“资料清单 / 任务清单”已补成真实对象入口，可直接打开对应课堂、学生、班级或资料详情，而不再只停在展示层。
...
- [x] Flutter: 个人中心页已迁入新版 `WorkspaceModuleShell`，顶部状态条、左侧模块导航与其他主模块保持一致，不再停留在旧 `Scaffold + AppBar` 壳层。
- [x] Flutter: 主工作台一级模块（工作台、题库、文档、学生、班级、课堂、导出、个人中心、设置）切换已改成无转场路由，避免工作台式模块跳转还沿用页面级默认动画。
...
- [x] Flutter: 首页“当前聚焦”已补“关联题库”入口，可按当前真实对象上下文把学科/学段/教材或课堂标题带入题库检索。
...
- [x] 打通学生模块第一条真实写入闭环：支持新增学生档案，并在工作页里即时回填与聚焦。
- [x] 打通学生创建关联编辑：可在新增学生时直接选择班级、课堂和资料，并一并写回真实数据。
- [x] Tooling: 真实写入验收脚本已覆盖学生创建关联编辑，可产出带初始班级承接语义的 live 学生图。
- [x] 打通班级模块第一条真实写入闭环：支持新增班级档案，并在工作页里即时回填与聚焦。
- [x] 打通课堂模块第一条真实写入闭环：支持新增课堂档案，并在工作页里即时回填与聚焦。
- [x] 打通学生模块第一条真实更新闭环：支持在学生详情页编辑基础档案并即时回看更新结果。
- [x] 打通班级模块第一条真实更新闭环：支持在班级详情页编辑基础档案并即时回看更新结果。
- [x] 打通课堂模块第一条真实更新闭环：支持在课堂详情页编辑基础档案并即时回看更新结果。
- [x] 打通学生模块第一条真实删除闭环：支持在学生详情页删除档案并返回学生工作页。
- [x] 打通班级模块第一条真实删除闭环：支持在班级详情页删除档案并返回班级工作页。
- [x] 打通课堂模块第一条真实删除闭环：支持在课堂详情页删除档案并返回课堂工作页。
- [x] Tooling: 新增“真实写入验收”脚本，可创建并更新临时学生/班级/课堂，然后产出工作页与详情页的 live 整页图。
- [x] Tooling: “真实写入验收”脚本已扩展到删除闭环，可产出学生/班级/课堂删除后的工作页 live 整页图。
- [x] 打通班级/课堂的重点学生关联编辑：可在编辑弹层里选择当前重点学生或反馈学生，并写回真实数据。
- [x] Tooling: 真实写入验收脚本已覆盖班级/课堂重点学生编辑，可产出带重点学生更新语义的 live 详情图。
- [x] 打通班级/课堂关联关系编辑：可在编辑弹层里直接选择关联课堂或关联班级，并写回真实数据。
- [x] Tooling: 真实写入验收脚本已覆盖班级/课堂关联关系编辑，可产出带“已关联课堂 / 已关联班级”语义的 live 详情图。
- [x] 打通班级/课堂文档绑定编辑：可在编辑弹层里直接选择关联资料，并写回真实数据。
- [x] Tooling: 真实写入验收脚本已覆盖班级/课堂文档绑定编辑，可产出带“已关联资料”语义的 live 详情图。
- [x] 打通学生关联关系编辑：可在编辑弹层里直接选择班级、课堂和资料，并写回真实数据。
- [x] Tooling: 真实写入验收脚本已覆盖学生关联关系编辑，可产出带“已关联班级 / 课堂 / 资料”语义的 live 学生详情图。
- [x] 打通班级成员关联编辑：可在班级编辑弹层里直接勾选班级成员，并同步写回学生档案中的班级关系。
- [x] Tooling: 真实写入验收脚本已覆盖班级成员关联编辑，可产出带“已关联成员”语义的 live 班级详情图。
- [x] 打通课堂承接学生编辑：可在课堂编辑弹层里直接勾选承接学生，并同步写回学生档案中的课堂关系。
- [x] Tooling: 真实写入验收脚本已覆盖课堂承接学生编辑，可产出带“已承接学生”语义的 live 课堂详情图。
- [x] 打通班级/课堂创建即关联：新建弹层已支持在创建时直接选择成员、关联对象与资料，避免必须二次编辑后才能建立真实关系。
- [x] 打通学生/班级/课堂归档状态流：详情页支持归档与恢复，本地数据库和列表查询已同步支持隐藏已归档档案。
- [x] Tooling: 真实写入验收脚本已覆盖学生/班级/课堂归档详情图，可固定产出“已归档但仍可回看/恢复”的 live 验收图。
- [x] Flutter: 题库已补第一条真实写入闭环，支持新建题目、编辑题目基础信息与题解主干、删除题目，并在题库页与题目详情页形成返回刷新闭环。
- [x] Tooling: 真实写入验收脚本已补题目创建/更新/删除整页图，题库写入能力也纳入固定 live 审计。
